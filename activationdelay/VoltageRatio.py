...
outfile = open("loadcell_log.csv", "w")
outfile.write(str(datetime.datetime.now())+ "\n")
outfile.write(str(start)+ "\n")
outfile.write("StartTime,SignalRecieved Time, elasped time,loadcell_analog,loadcell_kg\n")
...
def VoltageRatioChangeHandler(e, voltageRatio):
    global start
    global outfile
    now = time.clock() - start
    outfile.write(str(start) + "," + str(time.clock()) + "," + str(now) +  " , " + str(voltageRatio) + "\n")
...
print("Gathering data for 60 seconds...")
time.sleep(60)
